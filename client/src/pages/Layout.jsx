...
export default function Layout({ children, currentPageName }) {
  ...
  const mainClasses = isAdminPanel ? "min-h-screen bg-gray-50" : "min-h-screen bg-[#fdfaf6]";
              ...
              <div className="flex justify-between items-center h-20">
                <div className="flex items-center gap-4">
                  <Link to={createPageUrl("Cart")}>
                    <Button variant="outline" className="relative border-orange-200 hover:bg-orange-50 h-11 px-4">
                ...
                <nav className="hidden md:flex items-center gap-10">
                  <Link 
                    ...
                    className={`text-sm font-medium transition-colors hover:text-orange-500 ${
                      currentPageName === 'Homepage' ? 'text-orange-500' : 'text-gray-700'
                    }`}
                  >
                    דף הבית
                  </Link>
                  <Link 
                    ...
                    className={`text-sm font-medium transition-colors hover:text-orange-500 ${
                      currentPageName === 'Products' ? 'text-orange-500' : 'text-gray-700'
                    }`}
                  >
                    מוצרים
                  </Link>
                ...
                <Link to={createPageUrl("Homepage")} className="flex items-center gap-3 group">
                  <div>
                    <h1 className="text-2xl font-bold text-gray-800 text-right">NutHub</h1>
                    <p className="text-xs text-orange-500 -mt-1 tracking-wider">אגוזים וזרעים מובחרים</p>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-right">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <Link to={createPageUrl("Homepage")} className="block text-green-200 hover:text-white text-sm transition-colors">
                      דף הבית
                    </Link>
                    <Link to={createPageUrl("Products")} className="block text-green-200 hover:text-white text-sm transition-colors">
                      מוצרים
                    </Link>
                    <Link to={createPageUrl("Cart")} className="block text-green-200 hover:text-white text-sm transition-colors">
                      סל קניות
                    </Link>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-green-200 text-sm">
                    אגוזים וזרעים באיכות פרימיום מהחוות הטובות ביותר.
                    מביאים את הטוב שבטבע לשולחן שלכם.
                  </p>
                </div>
              </div>
              
              <div className="border-t border-green-700 mt-8 pt-8 text-center text-green-200 text-sm">
                <p>&copy; {new Date().getFullYear()} NutHub. כל הזכויות שמורות.</p>
              </div>
            </div>
          </footer>
        </>
